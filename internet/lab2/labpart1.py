...
def TaskOne():
    """
    Takes user input for a traffic signal colour. Based on the colour, it determines
    whether traffic is stopped or moving, and displays corresponding messages. It
    handles invalid inputs by prompting the user to re-enter the colour.
...
def findDaysForChirs():
    """
    Calculates and displays the number of days until the next Christmas Day by
    comparing the current date with the upcoming Christmas Day, accounting for
    years when Christmas has already passed. It then prints the result to the console.
...
def TaskTwo():
    """
    Demonstrates date and time calculations using Python's datetime module. It
    calculates and prints various time intervals, including one year from now, two
    years from now, a specific future date, and a past date three weeks ago.
...
def TaskThree():
    """
    Reads grocery items and their quantities from a text file, stores them in a
    dictionary, and then prompts the user to select a category. It displays the
    item and quantity if available, allowing the user to order multiple items with
